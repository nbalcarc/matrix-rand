...
use crate::{hadamard::HadamardMultiplier, monte::MonteMultiplier, tools::normalize_vector};
...
/// A two-hidden-layer neural network with customizable layer sizes.
pub struct NeuralNetworkAltered {
    sizes: (usize, usize, usize, usize),
    multipliers: Vec<MonteMultiplier>,
...
impl NeuralNetworkAltered {
    pub fn new(downscale: usize, sizes: (usize, usize, usize, usize)) -> Self {
        NeuralNetworkAltered {
            sizes,
            multipliers: vec![
                MonteMultiplier::new(sizes.0 / downscale),
                MonteMultiplier::new(sizes.1 / downscale),
                MonteMultiplier::new(sizes.2 / downscale),
                MonteMultiplier::new(sizes.3 / downscale),
            ],
    ...
    pub fn forward(&mut self, input: &[f32]) -> Vec<f32> {
        ...
        // first hidden layer
        let mut layer0 = vec![0.0; self.sizes.1];
        self.multipliers[0].multiply(&input, &self.weights0, (1, self.sizes.0, self.sizes.1), &mut layer0); //matrix mul
        for i in 0..layer0.len() { //apply bias + nonlinearity
            layer0[i] = (layer0[i] + self.bias0[i]).tanh();
        }
        ...
        // second hidden layer
        let mut layer1 = vec![0.0; self.sizes.2];
        self.multipliers[1].multiply(&layer0, &self.weights1, (1, self.sizes.1, self.sizes.2), &mut layer1); //matrix mul
        for i in 0..layer1.len() { //apply bias + nonlinearity
            layer1[i] = (layer1[i] + self.bias1[i]).tanh();
        }
        ...
        // output layer
        let mut output = vec![0.0; self.sizes.3];
        self.multipliers[2].multiply(&layer1, &self.weights2, (1, self.sizes.2, self.sizes.3), &mut output); //matrix mul
        for i in 0..output.len() { //apply bias + nonlinearity
            output[i] = (output[i] + self.bias2[i]).tanh();
        }
    ...
    pub fn backward(&mut self, input: &[f32], expected: &[f32]) -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, f32) {
        ...
        // delta second hidden layer
        let mut delta1 = vec![0.0; self.sizes.2];
        self.multipliers[3].multiply(&self.weights2, &delta2, (self.sizes.2, self.sizes.3, 1), &mut delta1);
        for i in 0..delta1.len() {
            delta1[i] *= layer1_z[i];
            if delta1[i].is_infinite() || delta1[i].is_nan() {
                todo!();
        ...
        // delta first hidden layer
        let mut delta0 = vec![0.0; self.sizes.1];
        self.multipliers[2].multiply(&self.weights1, &delta1, (self.sizes.1, self.sizes.2, 1), &mut delta0);
        for i in 0..delta0.len() {
            delta0[i] *= layer0_z[i];
            if delta0[i].is_infinite() || delta0[i].is_nan() {
                println!("{:?}", layer0_z[i]);
...
/// A two-hidden-layer neural network with customizable layer sizes.
pub struct NeuralNetworkAltered1 {
    sizes: (usize, usize, usize, usize),
    multipliers: Vec<HadamardMultiplier>,
...
impl NeuralNetworkAltered1 {
    pub fn new(downscale: usize, sizes: (usize, usize, usize, usize)) -> Self {
        NeuralNetworkAltered1 {
            sizes,
            multipliers: vec![
                HadamardMultiplier::new(sizes.0),
                HadamardMultiplier::new(sizes.1),
                HadamardMultiplier::new(sizes.2),
                HadamardMultiplier::new(sizes.3),
            ],
