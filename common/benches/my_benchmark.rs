...
fn benchmarks(c: &mut Criterion) {
    ...
    c.bench_function("monte float 1152", |x| x.iter(|| monte::multiply(&a_float, &b_float, sizes, 1152)));
    c.bench_function("monte float 768", |x| x.iter(|| monte::multiply(&a_float, &b_float, sizes, 768)));
    c.bench_function("monte float 384", |x| x.iter(|| monte::multiply(&a_float, &b_float, sizes, 384)));
    c.bench_function("monte float 192", |x| x.iter(|| monte::multiply(&a_float, &b_float, sizes, 192)));
}
